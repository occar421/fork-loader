...
it("should import child requests in parent's source", async () => {
  ...
  expect(parentModule1.source).toMatch(/import ".*?\?tag=foo&id=a";/);
  ...
  expect(parentModule2.source).toMatch(/import ".*?\?tag=foo&id=a";/);
  expect(parentModule2.source).toMatch(/import ".*?\?tag=foo&id=b";/);
});

it("should keep the source in forked stream.", async () => {
  const sourceFileName = "./fixtures/foo.js";
  const originalSource = fs
    .readFileSync(path.join(__dirname, sourceFileName))
    .toString();
  ...
  const forkedModule1 = status1
    .toJson()
    .modules.find(m => m.name.indexOf("id=a") > -1);
  ...
  const forkedModule2b = status2
    .toJson()
    .modules.find(m => m.name.indexOf("id=b") > -1);
